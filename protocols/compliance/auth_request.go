...
// to sender server in order to obtain digitalbits.toml file and signing key.
func (r *AuthRequest) VerifySignature(sender string) error {
	...
	if err != nil {
		return errors.Wrap(err, "Cannot get digitalbits.toml of sender domain")
	}

	if senderStellarToml.SigningKey == "" {
		return errors.New("No SIGNING_KEY in digitalbits.toml of sender")
	}
