...
func TestFrontier(t *testing.T) {
	...
	RunSpecs(t, "Package: github.com/digitalbitsorg/go/frontier")
}
...
var _ = Describe("Frontier", func() {
	...
	Describe("LoadAccount", func() {
		...
		It("failure response", func() {
			...
			Expect(err.Error()).To(HavePrefix("frontier error"))
			frontierError, ok := err.(*Error)
			Expect(ok).To(BeTrue())
			Expect(frontierError.Problem.Title).To(Equal("Resource Missing"))
		})
	...
	Describe("SubmitTransaction", func() {
		...
		It("failure response", func() {
			...
			Expect(err).NotTo(BeNil())
			Expect(err.Error()).To(ContainSubstring("Frontier error"))
			frontierError, ok := errors.Cause(err).(*Error)
			Expect(ok).To(BeTrue())
			Expect(frontierError.Problem.Title).To(Equal("Transaction Failed"))
		})
