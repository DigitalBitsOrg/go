...
func (bot *Bot) Pay(destAddress string) (*frontier.TransactionSuccess, error) {
	...
	v := <-channel
	switch tv := v.(type) {
	case frontier.TransactionSuccess:
		return &tv, nil
	case error:
		return nil, tv
	default:
...
func (bot *Bot) lockedPay(channel chan interface{}, destAddress string) (bool, *frontier.TransactionSuccess, error) {
	bot.lock.Lock()
...
func (bot *Bot) checkHandleBadSequence(err *frontier.Error) {
