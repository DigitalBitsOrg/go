...
| hash             | string | A hex-encoded SHA-256 hash of the transaction's [XDR](../../reference/xdr.md)-encoded form.                                                              |
...
To submit a new transaction to DigitalBits network, it must first be built and signed locally. Then you can submit a hex representation of your transaction’s [XDR](../../reference/xdr.md) to the `/transactions` endpoint. Read more about submitting transactions in [Post Transaction](../endpoints/transactions-create.md) doc.
