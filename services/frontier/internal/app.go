...
import (
	...
	"github.com/digitalbitsorg/go/clients/digitalbitscore"
	...
	"github.com/digitalbitsorg/go/services/frontier/internal/render/sse"
...
func (a *App) Serve() {
	...
	log.Infof("Starting frontier on %s (ingest: %v)", addr, a.config.Ingest)
...
// HorizonSession returns a new session that loads data from the frontier
...
// CoreSession returns a new session that loads data from the digitalbits core
// database. The returned session is bound to `ctx`.
...
func (a *App) Tick() {
	...
	// update ledger state and digitalbits-core info in parallel
	wg.Add(2)
