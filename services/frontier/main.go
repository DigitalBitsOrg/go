...
import (
	...
	"github.com/digitalbitsorg/go/services/frontier/internal"
	hlog "github.com/digitalbitsorg/go/services/frontier/internal/log"
)

var app *frontier.App
...
func init() {
	...
	viper.BindEnv("ruby-frontier-url", "RUBY_FRONTIER_URL")
	...
	rootCmd.Flags().String(
		"db-url",
		"",
		"frontier postgres database to connect with",
	)
	...
	rootCmd.Flags().String(
		"tls-cert",
		"",
		"The TLS certificate file to use for securing connections to frontier",
	)

	rootCmd.Flags().String(
		"tls-key",
		"",
		"The TLS private key file to use for securing connections to frontier",
	)

	rootCmd.Flags().Bool(
		"ingest",
		false,
		"causes this frontier process to ingest data from digitalbits-core into frontier's db",
	)
	...
	rootCmd.Flags().Uint(
		...
		0,
		"the minimum number of ledgers to maintain within frontier's history tables.  0 signifies an unlimited number of ledgers will be retained",
	)

	rootCmd.Flags().Uint(
		"history-stale-threshold",
		0,
		"the maximum number of ledgers the history db is allowed to be out of date from the connected digitalbits-core db before frontier considers history stale",
	)
...
func initApp(cmd *cobra.Command, args []string) {
	...
	var err error
	app, err = frontier.NewApp(config)
