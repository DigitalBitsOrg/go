...
Bifrost is highly available and secure Bitcoin/Ethereum → DigitalBits bridge. It allows users to move BTC/ETH to DigitalBits network and then trade them to other tokens or participate in ICOs (initial coin offering).

It solves many problems connected to moving tokens to DigitalBits network:

* Security:
  * Developers don’t have access to users’ DigitalBits keys.
...
Download the binary from [the release page](https://github.com/digitalbitsorg/go/releases/tag/bifrost-v0.0.1) and use it with it's [Bifrost JS SDK](https://github.com/digitalbitsorg/bifrost-js-sdk).

## How it works

1. User opens the web app implemented using [Bifrost JS SDK](https://github.com/digitalbitsorg/bifrost-js-sdk).
1. User is presented with her public and private DigitalBits keys where Bitcoin/Ethereum will be sent.
1. User selects what cryptocurrency she wants to move to DigitalBits network.
1. A receiving Bitcoin/Ethereum address is generated.
1. User sends funds in Bitcoin/Ethereum network.
1. Bifrost listens to Bitcoin and Ethereum network events. When payment arrives it creates a DigitalBits [account](https://developer.digitalbits.io/guides/concepts/accounts.html) for the user.
1. User creates a [trust line](https://developer.digitalbits.io/guides/concepts/assets.html) to BTC/ETH issued by Bifrost account.
1. Bifrost sends corresponding amount of BTC/ETH to user's DigitalBits account.
1. As an optional step, web application can also exchange BTC/ETH to other token in DigitalBits network at a given rate.

## Demo

We use Ethereum Ropsten test network for this demo because it's faster that Bitcoin testnet.

https://bifrost.testnet.digitalbits.io/
...
1. Now open Bifrost demo: https://bifrost.testnet.digitalbits.io/ It will display an Ethereum address for sending your test ETH.
...
* `digitalbits`
  ...
  * `signer_secret_key` - issuer's secret key if only one instance of Bifrost is deployed OR [channel](https://developer.digitalbits.io/guides/channels.html)'s secret key if more than one instance of Bifrost is deployed. Signer's sequence number will be consumed in transaction's sequence number.
  * `horizon` - URL to [frontier](https://github.com/digitalbitsorg/go/tree/master/services/frontier) server
  * `network_passphrase` - DigitalBits network passphrase (`Live DigitalBits Network ; March 2018` for production network, `Test DigitalBits Network ; December 2017` for test network)
  * `starting_balance` - DigitalBits XDB amount issued to created account (41 by default)
* `database`
...
* Make sure "Sell [your token] for BTC" and/or "Sell [your token] for ETH" exist in DigitalBits production network. If not, you can create an offer by sending a transaction with `manage_offer` operation.
